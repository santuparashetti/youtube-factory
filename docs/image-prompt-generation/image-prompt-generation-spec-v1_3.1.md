# Image Prompt Generation & Escalation Spec (v3.1)
...
If `scene_importance` in {hero, climax, opening_hook, closing_frame} → skip directly to **Step 3**
...
| `text_watermark` **(confirmed real key name — not `text`)** | Fails if text or a watermark appears in the image when the prompt requested neither — text and watermark detection are merged into one constraint in the real implementation, not two separate ones |
...
9. **[Minor drift — confirmed]** Vision QA JSON Schema Gap: `text` — confirmed via doc-accuracy check.
   ...
   table and §5's remediation table below should reference `text_watermark`, not `text`, as the actual
   field name — updated below.
10. **[Resolved — confirmed]** Vision QA JSON Schema Gap: `overall_score` — confirmed. `overall_score`
    ...
    response elsewhere. §4's numeric thresholds are fully wired to a real field.
...
12. **[New — decision needed]** Internal contradiction: §4 Step 0 lists hero-scene classification as
    `{hero, climax, opening_hook, closing_frame}` (four values), but the confirmed implementation of
    `_is_hero_scene` only checks `("HERO", "CLIMAX")` (two values) — this doc has not decided which is
    correct. Either:
    - **(a)** narrow §4 Step 0 to match the implemented two-value set, or
    - **(b)** treat this as a real gap and extend `_is_hero_scene` to also recognize
      `opening_hook`/`closing_frame` scenes, if those are meant to get the same "skip tier 1, go
      straight to Qwen-Image" treatment as hero/climax scenes.
    This is a product decision (which scenes deserve the escalation-skip treatment), not something to
    resolve by silently picking one side — flagged here pending that call.
