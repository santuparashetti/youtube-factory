...
# APPENDIX: Provider Abstraction (application layer — not LLM instructions)

This section is a codebase architecture decision, separate from the
editorial rules above. It should **not** be included when the
editorial spec above is pasted as a system prompt to an LLM.

The subtitle pipeline shall depend only on a `SubtitleEditorProvider`
interface, never call any LLM SDK directly, and support swappable
implementations via configuration:

-   `ClaudeSubtitleEditor`
-   `GPTSubtitleEditor`
-   `GeminiSubtitleEditor`
-   `MockSubtitleEditor` (for testing, deterministic output, no live API
    calls)

Business logic (document reconstruction, validation, retry-on-mismatch,
scoring, debug file generation) must live outside any specific provider
implementation. The pipeline must be swappable between providers
through configuration alone, without changing business logic.
