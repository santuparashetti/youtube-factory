...
2. Use the section map as a candidate pool, not a coverage checklist.
   - The section map tells you which source handled each beat more effectively.
     Use that signal to decide WHAT TO DRAW FROM — not as a mandate to include
     every listed section. A section that duplicates another beat, weakens the arc,
     or would push you over the word ceiling should be cut regardless of its map winner.
   - When you do include a section: draw from the winning source's approach — its
     angle, imagery, or framing — but rewrite it for the final voice and arc.
     The core insight and emotional turn must land; the exact words do not need to.
   - You may draw from A for a section the map assigned to B, or from neither, if
     that choice better serves the final arc. Source loyalty is subordinate to arc quality.
...
4. Preserve the NARRATIVE ARC above all else. The arc is the pipeline formula above —
   hook through memorable final line. Taking the "stronger" version of a section means
   finding a better way to deliver the SAME story beat, not replacing what that beat
   accomplishes in the arc. Never let a locally better passage break the cause-and-effect
   chain that carries the viewer from hook to close.
5. Hook rule — the hook must end on tension or a striking image. Do not write a sentence
   that explains the tension the hook just created. If the hook ends with a question that
   explains itself, cut that question. Let the tension hang.
6. Single visual world — once a visual world is established, every metaphor for the
   remainder of the script must live inside that world. A lamp, a sugarcane plant, a
   river — these introduce new visual worlds and must be cut or converted into the
   established metaphor world. If a section from either source introduces a new visual
   world, do not include that section.
7. No repeated beats — if two sections from the combined sources make the same point,
   include only the stronger one. No two paragraphs in the output may make the same point.
8. Preserve the opening hook from whichever script the section map designates as
